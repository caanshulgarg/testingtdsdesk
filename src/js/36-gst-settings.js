...
function viewGstRegs(b, regs){
  ...
  if (regs.length) h += '<div class="bk-tablewrap"><table class="bk-table compact"><thead><tr><th>GSTIN</th><th>State</th><th>Filing type now</th><th>Portal username</th><th>Taken from</th><th></th></tr></thead><tbody>' +
    regs.map(g => { const reg = g.slice(0, 2), src = GSTRegs.source(g, b), only = src.length === 1 && src[0] === "added here", months = GSTR.months();
      return '<tr><td><b>' + esc(g) + "</b></td><td>" + esc(GSTRegs.state(g)) + "</td><td>" + esc(GSTSet.typeLabel(GSTSet.typeOf(months[months.length - 1] || GSTF.today().slice(0, 7).replace("-", ""), reg))) + "</td><td>" + (GSTSet.peek(reg).portalUser ? esc(GSTSet.peek(reg).portalUser) + ' <button class="linkbtn" data-gsetuser="' + reg + '">change</button>' : '<button class="linkbtn" data-gsetuser="' + reg + '">type it</button>') +
...
if (typeof document !== "undefined"){
  ...
  document.addEventListener("click", e => {
    ...
    if (t.dataset.gsetuser){ S.gsetReg = t.dataset.gsetuser; render();
      const i = document.querySelector('input[data-gset="puser"][data-greg="' + t.dataset.gsetuser + '"]'); if (i){ i.scrollIntoView({block: "center", behavior: "smooth"}); i.focus(); } return; }
