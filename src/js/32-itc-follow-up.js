...
const ITCT = {
  ...
  letter(reg, gstin, party, items){
    ...
    return {text: t, n: w.length + d.length + n.length};
  },
  suppliers(reg, items){
    const m = {};
    items.filter(x => x.open && ((x.cat === "waiting" && x.act === "follow") || (x.cat === "diff" && x.act === "supplier") || (x.cat === "ournote" && x.act === "follow"))).forEach(x => {
      const k = x.gstin || x.supplier, s = m[k] = m[k] || {key: k, gstin: x.gstin, party: x.supplier, items: [], tax: 0, oldest: x.date, deadline: x.deadline};
      s.items.push(x); s.tax = r2(s.tax + (x.cat === "diff" ? Math.abs(x.gap) : x.tax)); if (String(x.date) < String(s.oldest)) s.oldest = x.date; if (x.deadline && (!s.deadline || x.deadline < s.deadline)) s.deadline = x.deadline;
    });
...
function viewItcFollow(b){
  ...
  h += '<div class="bk-tablewrap"><table class="bk-table compact fixed"><colgroup>' + [11, 16, 12, 11, 8, 9, 8, 14, 11].map(w => '<col style="width:' + w + '%">').join("") + "</colgroup><thead><tr>" + cols.map((c, i) => "<th" + (i === 5 ? ' class="n"' : "") + ">" + c + "</th>").join("") + "</tr></thead><tbody>" +
    list.slice(0, 3000).map(x => {
      ...
      const twoB = x.where === "Tally" ? '<span class="' + (x.covered ? "bad" : "nr") + '">' + esc(x.covered ? "not in 2B" : "no 2B yet") + "</span>" : esc(GSTR.label(x.ym2b || x.ym)) + (x.claimedIn ? '<div class="nr">taken then</div>' : x.reason ? '<div class="nr">' + esc(x.reason) + "</div>" : x.ims ? '<div class="nr">IMS: ' + esc(x.ims) + "</div>" : "");
...
async function itctExcel(){
  ...
  const ims = R.items.filter(x => x.where === "2B" && (x.cat === "tobook" || x.cat === "suppcn" || x.cat === "nocredit")).map(x => [x.supplier, x.gstin, x.no, d(x.date), GSTR.label(x.ym2b), x.tax,
    x.cat === "suppcn" ? (x.act === "reject" ? "Reject" : "Accept") : x.act === "reject" ? "Reject" : x.act === "pending" ? "Pending" : "Accept", actLabel(x), x.ims || ""]);
