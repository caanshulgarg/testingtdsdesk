...
with sync_playwright() as p:
    ...
    pg.check('input[data-gstf="r37on"]'); pg.wait_for_timeout(4000); t = pg.inner_text("#app")
    ...
    ok("Its figures are the ones to pay" in t, "says the portal's interest and late fee are the ones to pay")
    pg.fill('input[data-gstf="r3b"]', "2025-12-05"); pg.press('input[data-gstf="r3b"]', "Tab"); pg.wait_for_timeout(3000)
    t = pg.inner_text("#app")
    ok(pg.evaluate("S.books.gstFiled['07']['202510'].r3b") == "2025-12-05" and "Interest, estimate" in t, "a late filing date brings the late fee and section 50 interest")
    pg.fill('input[data-gstf="portalInt"]', "12345"); pg.press('input[data-gstf="portalInt"]', "Tab"); pg.wait_for_timeout(2500)
    ok(pg.evaluate("S.books.gstFiled['07']['202510'].portalInt") == 12345 and "estimate ₹" in pg.inner_text("#app"), "the portal's interest typed in is kept, and the estimate shown against it")
