...
with sync_playwright() as p:
    ...
    pg.select_option('select[data-gset="basis"][data-greg="07"]', "books"); pg.wait_for_timeout(700); pg.click('[data-cbx="yes"]'); pg.wait_for_timeout(1500)
    pg.evaluate("S.tab = 'books'; render()"); pg.wait_for_timeout(4000)
    ok("Held back" not in pg.inner_text("#app") and pg.evaluate("S.books.itcBasis['07']") == "books", "switched to the books basis in GST settings")
    pg.evaluate("S.tab = 'gstset'; render()"); pg.wait_for_timeout(2000)
    pg.select_option('select[data-gset="basis"][data-greg="07"]', "2b"); pg.wait_for_timeout(700); pg.click('[data-cbx="yes"]'); pg.wait_for_timeout(1500)
