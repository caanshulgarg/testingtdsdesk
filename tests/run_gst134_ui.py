...
with sync_playwright() as p:
    ...
    ok(pg.locator('input[data-gstopen="igst"]').count() == 1, "the first month asks for the credit ledger balance from the portal")
    before = pg.evaluate("GSTR.threeB('202504','09').pay.cash.igst + GSTR.threeB('202504','09').pay.cash.cgst")
    pg.select_option('select[data-gstreg]', "09"); pg.wait_for_timeout(2000)
    pg.fill('input[data-gstopen="cgst"]', "500000"); pg.press('input[data-gstopen="cgst"]', "Tab"); pg.wait_for_timeout(2500)
    after = pg.evaluate("GSTR.threeB('202504','09').pay.cash.igst + GSTR.threeB('202504','09').pay.cash.cgst")
    ok(pg.evaluate("S.books.gstOpen['09'].cgst") == 500000 and after < before, "an opening balance typed reduces cash payable: %s to %s" % (before, after))
    pg.select_option('select[data-gstreg]', "07"); pg.wait_for_timeout(1500)
